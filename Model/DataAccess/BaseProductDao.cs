...
{
    protected async Task<T?> Get<T>(QueryDefinition queryDefinition, string type)
    {
    ...
    protected async Task<List<T>?> GetList<T>(QueryDefinition queryDefinition, string type)
    {
        var container = cosmosClient.GetContainer("GoodStuff", "Products");
        var queryRequestOptions = new QueryRequestOptions
        {
