...
public class CosmosRepository<TProduct>(CosmosClient cosmosClient)  where TProduct : BaseProductModel, IRepository<TProduct>, new()
{
    private readonly Container _container = cosmosClient.GetContainer("GoodStuff", "Products");
    
    public async Task<IEnumerable<TProduct>> GetAllAsync()
    {
        BaseProductModel a = new TProduct();
    ...
    public async Task<TProduct?> GetById(string id)
    {
